...
import {decryptPassword, encryptPassword} from '../cryptography/keystore';
  ...
  public static async from(
    ...
    if (typeof data === 'string') {
      entropy = mnemonicToEntropy(
        Buffer.from(await decryptPassword<string>(password, data)).toString(
          'utf-8',
        ),
      );
    } else if (data.v === 1) {
      entropy = toEntropy(await decryptPassword<string>(password, data.data));
    } else if (data.v === 2) {
      const {entropy: entropySerialized, importedKeypairs} =
        await decryptPassword<V2DecryptedDataType>(password, data.data);
  ...
  public async encrypt(password: string) {
    const dataToEncrypt: V2DecryptedDataType = {
      ...
      importedKeypairs: this.importedKeypairs.map(aKeypair => {
        return aKeypair.export();
      }),
    };
    return {
      v: LATEST_VAULT_VERSION,
      data: await encryptPassword(password, dataToEncrypt),
    };
  }
