...
import {getRandomEntropy, toEntropy} from '../../ulti/bip39';
...
class VaultStorageClass {
  ...
  public async create(password: string, importedEntropy?: string) {
    ...
    await setToStorage(VAULT_KEY, await vault.encrypt(password));
    vault = null;
  }

  public async unlock(password: string) {
    const encryptedVault = await getFromStorage(VAULT_KEY);
    ...
    this.#vault = await Vault.from(password, encryptedVault, async aVault =>
      setToStorage(VAULT_KEY, await aVault.encrypt(password)),
    );
  ...
  public async lock() {
    this.#vault = null;
    await setToStorage(EPHEMERAL_PASSWORD_KEY, null);
    await setToStorage(EPHEMERAL_VAULT_KEY, null);
  }

  public async revive(): Promise<boolean> {
    let unlocked = false;
    const rndPass = await getFromStorage(EPHEMERAL_PASSWORD_KEY);
    if (rndPass) {
      const ephemeralPass = makeEphemeraPassword(rndPass);
      const ephemeralEncryptedVault = await getFromStorage(EPHEMERAL_VAULT_KEY);
  ...
  public async clear() {
    await this.lock();
    await setToStorage(VAULT_KEY, null);
  }

  public async isWalletInitialized() {
    return !!(await getFromStorage(VAULT_KEY));
  }
  ...
  public async importKeypair(
    ...
    await setToStorage(VAULT_KEY, await this.#vault.encrypt(password));
    await this.updateSessionStorage();
  ...
  private async updateSessionStorage() {
    ...
    const ephemeralPass = makeEphemeraPassword(rndPass);
    await setToStorage(EPHEMERAL_PASSWORD_KEY, rndPass);
    await setToStorage(
      EPHEMERAL_VAULT_KEY,
